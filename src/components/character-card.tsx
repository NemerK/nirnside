      ...
      <Link href={`/characters/${encodeURIComponent(c.id)}`} className="flex flex-1 flex-col p-4 pb-2">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="shrink-0 text-right">
            <div className="text-sm font-semibold text-fg">{c.level >= 50 ? "Max" : `Lv ${c.level}`}</div>
        ...
        <div className="mt-3 flex items-end justify-between gap-2 border-t border-border pt-2 text-xs text-fg-subtle">
          ...
          {seen ? <span className="shrink-0 tabular-nums text-fg-muted">{formatGold(c.gold ?? 0)}</span> : null}
        </div>
      </Link>
      {roles && (
