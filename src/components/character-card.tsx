...
import { ALLIANCE_ACCENT, timeAgo } from "@/lib/format";
...
export function CharacterCard({ character: c }: { character: Character }) {
  ...
  return (
    <Link
      ...
      className="group relative flex flex-col overflow-hidden rounded-xl border border-border bg-surface/70 p-4 transition-colors hover:border-accent/50 hover:bg-surface-2"
    >
      ...
      <div className="mt-3 border-t border-border pt-2 text-xs text-fg-subtle">
        {seen ? `Snapshot ${timeAgo(seen)}` : "Not logged in since install — log this character out to fill in"}
      </div>
    </Link>
  );
}
